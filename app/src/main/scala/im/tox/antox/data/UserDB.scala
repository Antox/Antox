...
import im.tox.antox.data.UserDB.DatabaseHelper
import im.tox.antox.utils.{BriteScalaDatabase, Constants, DatabaseUtil}
...
class UserDB(ctx: Context) {
  ...
  def addUser(username: String, password: String) {
    ...
    val defaultStatusMessage = ctx.getResources.getString(R.string.default_status_message)
